...
func TestForeignKeyEnforcementRoundTrip(t *testing.T) {
	...
	for _, enforcement := range []string{"", "ENFORCED", "NOT ENFORCED", "ON DELETE NO ACTION", "ON DELETE NO ACTION ENFORCED", "ON DELETE NO ACTION NOT ENFORCED", "ON DELETE CASCADE ENFORCED"} {
		t.Run(enforcement, func(t *testing.T) {
			...
			require.Equal(t, strings.Contains(enforcement, "NOT ENFORCED"), strings.Contains(ddls[1], "NOT ENFORCED"))
